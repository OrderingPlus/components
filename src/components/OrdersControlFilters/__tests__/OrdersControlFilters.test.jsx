...
import { OrdersControlFilters } from '../index'

describe('OrdersControlFilters', () => {
  beforeEach(() => orders.reset())

  it('loads dashboard controls on mount', async () => {
    renderController(OrdersControlFilters, {})
    await waitFor(() => {
      expect(lastControllerProps.controlsState.loading).toBe(false)
    })
    expect(lastControllerProps.controlsState.drivers).toHaveLength(1)
    expect(lastControllerProps.controlsState.paymethods).toHaveLength(1)
  })
})
